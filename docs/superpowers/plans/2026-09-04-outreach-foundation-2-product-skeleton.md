...
**Amendments from the Task 9 review (applied in a fix wave):** the `returnTo` rule lives once, in `@cti/contracts` (`SAFE_RETURN_TO`, `isSafeReturnTo`; the server's `state.ts` and both SPA routes consume it); invalid `returnTo` values are stripped by `validateSearch` (`.catch(undefined)`) so the sign-in page fails soft; `team-page.tsx` renders `toggleAdmin.error` and maps `CANNOT_DEMOTE_SELF`/`MEMBER_NOT_FOUND`; `switchTenant` clears the React Query cache; `lib/api.ts` clears the in-memory session on any 401 (except the handoff exchange) and notifies `AuthProvider` through `setUnauthorizedHandler`, and `main.tsx` calls `router.invalidate()` whenever `isAuthenticated` changes so the guard redirects immediately; the callback route also strips an invalid `returnTo`; `shadcn` is pinned exactly (`4.21.0`); the callback effect depends on `auth.completeHandoff` with a started-latch (single fetch under StrictMode); router-level tests cover the guard redirect and hostile `returnTo` values; the `shadcn` package stays a devDependency because the generated `index.css` imports `shadcn/tailwind.css` at build time (documented in `components.json`); `cn` is a local `clsx` + `tailwind-merge` helper; the unused `select.tsx` is removed.
