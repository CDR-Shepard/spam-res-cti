...
describe('classify — inbound rows', () => {
  ...
  it('an inbound call still in progress is connected, not missed — and says "In progress", not "Answered"', () => {
    const live: CallRow = row({ direction: 'inbound', status: 'in_progress', durationSeconds: null, answeredAt: null, inboundVoicemailUrl: null });
    expect(classify(live)).toBe('connected');
    const html = renderToStaticMarkup(<RecentCallRow call={live} />);
