...
describe('migration 0044_inbound_messages', () => {
  ...
  it('the Drizzle schema names the same columns, defaults and nullability as the SQL', () => {
    ...
    expect(Object.values(c).map((col) => col.name).sort()).toEqual([
      'attempts', 'backfill', 'body', 'created_at', 'emailed_at', 'from_e164', 'id', 'last_error', 'message_sid',
      'next_attempt_at', 'num_media', 'org_id', 'received_at', 'sf_task_id', 'status', 'to_e164', 'updated_at', 'user_id',
    ]);
    ...
    for (const col of [c.userId, c.sfTaskId, c.emailedAt, c.lastError]) expect(col.notNull).toBe(false);
