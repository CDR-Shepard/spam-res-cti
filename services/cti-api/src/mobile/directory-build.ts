 ...
 * hardcoded), and the object itself may not exist in every org. Mirrors
 * `findByPhone`'s tolerance: a describe failure — 404, network error, or a
 * 200 with no `type === 'phone'` field — skips the Deal sweep with exactly
 * one warning rather than failing the whole snapshot.
 */
async function sweepDeals(deps: SweepDeps, userId: string): Promise<RawEntry[]> {
  ...
  try {
    ...
    if (res.status >= 400) throw new Error(`describe failed (${res.status})`);
    ...
    console.warn('[directory-build] Deal__c describe unavailable; skipping Deal sweep', (err as Error).message);
    return [];
  }
 ...
 *  dropped silently. A Deal__c describe failure skips Deals only — Leads and
 *  Opportunities are unaffected (see `sweepDeals`). */
...
/** One (org, connected user) pair per tick — the admin when one is connected,
 *  else whichever connected user comes back first. Pure so the preference
...
export function pickActingUsers(rows: ReadonlyArray<ActingUserCandidate>): Array<{ orgId: string; userId: string }> {
  ...
  for (const r of rows) {
    ...
    if (!cur || (r.isAdmin && !cur.isAdmin)) byOrg.set(r.orgId, r);
  }
